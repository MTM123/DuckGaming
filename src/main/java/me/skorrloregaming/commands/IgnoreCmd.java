		...
		if (args.length == 0) {
			...
			if (Server.getIgnoredPlayers().containsKey(player.getUniqueId())) {
				...
				if (existingIgnore.getName().toString().equals(targetPlayer.getName().toString())) {
					...
					player.sendMessage($.Legacy.tag + ChatColor.RED + "Success. " + ChatColor.GRAY + "You are no longer ignoring the specified player. If you had an issue while using the /ignore feature, please let the server administrators know about the issue.");
					return true;
				}
			}
			Server.getIgnoredPlayers().put(player.getUniqueId(), targetPlayer.getUniqueId());
			player.sendMessage($.Legacy.tag + ChatColor.RED + "Success. " + ChatColor.GRAY + "You are now ignoring the specified player. This will only last until the next server update and/or restart.");
		}
		return true;
	}
