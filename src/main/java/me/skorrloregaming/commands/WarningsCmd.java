	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (args.length == 0) {
			...
			if (Server.getInstance().getPlugin().getConfig().contains(configPath)) {
				warnings = Integer.parseInt(Server.getInstance().getPlugin().getConfig().getString(configPath));
				if (sender instanceof Player) {
					int invSize = 9;
					...
					for (String key : Server.getInstance().getPlugin().getConfig().getConfigurationSection("warning." + ipAddress).getKeys(false)) {
						if (key.equals("count"))
							continue;
			...
			if (Server.getInstance().getBanConfig().getData().contains(ipAddress.replace(".", "x"))) {
