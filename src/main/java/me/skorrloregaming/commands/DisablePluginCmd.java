...
public class DisablePluginCmd implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		Plugin plugin = ServerGet.get().getPlugin().getServer().getPluginManager().getPlugin(args[0]);
		if (!(plugin == null)) {
			ServerGet.get().getPlugin().getServer().getPluginManager().disablePlugin(plugin);
