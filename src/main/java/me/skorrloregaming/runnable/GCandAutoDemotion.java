		...
		if (player.hasPlayedBefore() || player.isOnline()) {
			if (ServerGet.get().getPlugin().getConfig().contains(configPath + ".username")) {
				...
				if (rankId > -1 && rankId < 3) {
					long totalPlaytime = LinkServerGet.get().getPlaytimeManager().getStoredPlayerPlaytime(player);
					long[] range = LinkServerGet.get().getPlaytimeManager().getRangeOfStoredPlayerPlaytime(player, dayOfYear - 6, dayOfYear + 1);
					...
					if (rank.equals("admin")) {
						if (totalPlaytime > managerPlaytimeRequirement) {
							if (Link$.validRanks.contains("manager")) {
								...
								LinkServerGet.get().getRedisMessenger().broadcast(RedisChannel.CHAT, message);
								Logger.info(rawMessage);
								LinkServerGet.get().getRedisDatabase().set("rank", player.getUniqueId().toString(), "manager");
